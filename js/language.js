...
async function changeLanguage(lang) {
  try {
    ...
    // 7. Update redNote label and tooltip if present
    const redNoteLink = document.querySelector('[aria-label="redNote Profile"]');
    if (redNoteLink && langData.redNoteTooltip) {
      redNoteLink.setAttribute('data-tooltip', langData.redNoteTooltip);
      const span = redNoteLink.querySelector('span');
      if (span) span.textContent = langData.redNoteTooltip;
    }
